import { Users, Trophy, Video, Gamepad2, Menu, Home, Layout, Settings } from "lucide-react";
    ...
    const Index = () => {
        ...
        return (
            <>
                <Head title="Welcome" />
                {/* Hero Section */}
                <div className="relative textcolorsidebar">
                    {/* Mobile Toggle Button - Only visible when sidebar is collapsed on mobile */}
                    {isMobile && isCollapsed && (
                        <button
                            onClick={() => setIsCollapsed(false)}
                            className=" fixed left-4 top-4 z-50 rounded-lg bg-sidebar-bg p-2 text-sidebar-text hover:bg-sidebar-hover transition-colors duration-200"
                        >
                            <Menu size={24} />
                        </button>
                    )}
                    ...
                    <div
                        className={`fixed left-0 top-0 h-screen bg-sidebar-bg transition-all duration-300 ease-in-out z-40 ${isCollapsed ? "w-20" : "w-64"
                            } ${isMobile && isCollapsed ? "-translate-x-full" : "translate-x-0"}`}
                    >
                        <div className="flex h-16 items-center justify-between px-4">
                            {!isCollapsed && (
                                <span className="text-xl font-semibold text-sidebar-text">Logo</span>
                            )}
                            <button
                                onClick={() => setIsCollapsed(!isCollapsed)}
                                className="rounded-lg p-2 text-sidebar-text hover:bg-sidebar-hover transition-colors duration-200"
                            >
                                <Menu size={24} />
                            </button>
                        </div>
                        ...
                        {menuItems.map((item) => (
                                <Link
                                    key={item.label}
                                    href={item.path}
                                    className="flex items-center px-4 py-3 text-sidebar-text hover:bg-sidebar-hover transition-colors duration-200"
                                >
